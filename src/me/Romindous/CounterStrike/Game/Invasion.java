...
public class Invasion extends Arena {
	...
	public Invasion(final String name, final byte min, final byte max, final BaseBlockPosition[] TSps, final BaseBlockPosition[] CTSps, final World w, final BaseBlockPosition ast, final BaseBlockPosition bst, final boolean rnd) {
	...
	public boolean addPl(final Shooter sh) {
		...
		case BEGINING:
			...
			for (final Shooter s : shtrs.keySet()) {
				...
				PacketUtils.sendRvsNmTg(Main.ds.bh().a(sh.nm).networkManager, Team.CTs, Main.ds.bh().a(p.getName()), Team.CTs, "§7<§d" + name + "§7> ", " §7[-.-]", Team.CTs.clr);
			}
			break;
		case ROUND:
			...
			PacketUtils.sendNmTg(sh, Team.CTs.icn + " ", " §7[0-0]", Team.CTs.clr);
			for (final Shooter s : shtrs.keySet()) {
				final Player p = (Player) s.inv.getHolder();
				...
				PacketUtils.sendRvsNmTg(Main.ds.bh().a(pl.getName()).networkManager, Team.CTs, Main.ds.bh().a(p.getName()), Team.CTs, Team.CTs.icn + " ", " §7[0-0]", Team.CTs.clr);
			}
			if (shtrs.size() > 1) {
				bst.i(true);
				bst.n(true);
	...
	private void cntStrt() {
		...
		for (final Shooter sh : shtrs.keySet()) {
			...
			gameScore(sh);
			p.teleport(Main.getNrLoc(CTSps[Main.srnd.nextInt(CTSps.length)], w));
			PacketUtils.sendNmTg(sh, Team.CTs.icn + " ", " §7[" + sh.kls + "-" + sh.dths + "]", Team.CTs.clr);
