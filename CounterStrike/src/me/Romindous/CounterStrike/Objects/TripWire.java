...
public class TripWire {
	...
	public TripWire(final Block[] bs, final Pair<Shooter, Arena> pr, final BlockFace bf, byte i) {
		...
		this.nt = null;
		eif = new EntityItemFrame(EntityTypes.R, PacketUtils.getNMSWrld(bs[0].getWorld().getName()));
		eif.j(true);
		eif.e(true);
		...
		for (final Entry<Shooter, Team> e : pr.getSecond().shtrs.entrySet()) {
			if (e.getValue() == tm) {
				shwNd(Main.ds.bg().a(e.getKey().inv.getHolder().getName()).b, pe, pm);
			}
		}
	}
	...
	public void trgr(final Location loc) {
		...
		if (this.nt != null) {
			final Material m; 
			final World w = PacketUtils.getNMSWrld(loc.getWorld().getName());
	...
	public void chngNt(final ItemStack it, final Arena ar) {
		...
		for (final Entry<Shooter, Team> e : ar.shtrs.entrySet()) {
			if (e.getValue() == tm) {
				Main.ds.bg().a(e.getKey().inv.getHolder().getName()).b.a(pp);
			}
		}
	}
	...
	public void hdNdAll(final Arena ar) {
		...
		for (final Entry<Shooter, Team> e : ar.shtrs.entrySet()) {
			if (tm == e.getValue()) {
				Main.ds.bg().a(e.getKey().inv.getHolder().getName()).b.a(pd);
			}
		}
	}
