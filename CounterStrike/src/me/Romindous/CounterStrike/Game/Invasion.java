...
public class Invasion extends Arena {
	...
	public Invasion(final String name, final byte min, final byte max, final BaseBlockPosition[] TSps, final BaseBlockPosition[] CTSps, final World w, final BaseBlockPosition ast, final BaseBlockPosition bst) {
		...
		as.setSilent(true);
		as.setCustomName(IChatBaseComponent.a("§7Точка §bA§7: §d100%"));
		as.setCustomNameVisible(false);
		as.setPosition(ast.getX() + 0.5d, ast.getY() - 0.4d, ast.getZ() + 0.5d);
		wm.addEntity(as);
		this.ast = as;
		final EntityShulkerBullet bs = new EntityShulkerBullet(EntityTypes.az, wm);
		bs.setInvulnerable(true);
		bs.setNoGravity(true);
		bs.setInvisible(true);
		bs.setSilent(true);
		bs.setCustomName(IChatBaseComponent.a("§7Точка §6B§7: §d100%"));
		bs.setCustomNameVisible(false);
		bs.setPosition(bst.getX() + 0.5d, bst.getY() - 0.4d, bst.getZ() + 0.5d);
		wm.addEntity(bs);
		this.bst = bs;
	...
	public boolean addPl(final Shooter sh) {
		...
		case BEGINING:
			...
			for (final Shooter s : shtrs.keySet()) {
				...
				PacketUtils.sendRvsNmTg(PacketUtils.getNMSPlr((Player) sh.inv.getHolder()).b, Team.CTs, PacketUtils.getNMSPlr(p), Team.CTs, "§7<§d" + name + "§7> ", " §7[-.-]", Team.CTs.clr);
			}
			break;
		case ROUND:
			...
			for (final Shooter s : shtrs.keySet()) {
				...
				PacketUtils.sendRvsNmTg(PacketUtils.getNMSPlr(pl).b, Team.CTs, PacketUtils.getNMSPlr(p), Team.CTs, Team.CTs.icn + " ", " §7[0-0]", Team.CTs.clr);
			}
			if (shtrs.size() > 1) {
				bst.setGlowingTag(true);
				bst.setCustomNameVisible(true);
				Main.getBBlock(bst.getChunkCoordinates().c(0, 1, 0), w).setType(Material.WARPED_PRESSURE_PLATE, false);
			}
	...
	public boolean rmvPl(final Shooter sh) {
		...
		case ENDRND:
			...
			if (shtrs.size() == 0) {
				...
				bst.setGlowingTag(false);
				bst.setCustomNameVisible(false);
				Main.getBBlock(bst.getChunkCoordinates().c(0, 1, 0), w).setType(Material.AIR, false);
			} else {
				cnt = Math.max(((isDay ? 80 : 48) - ccl) / shtrs.size(), 1);
				for (final Shooter s : shtrs.keySet()) {
					final Player p = (Player) s.inv.getHolder();
	...
	private void cntStrt() {
		...
		ast.setGlowingTag(true);
		ast.setCustomNameVisible(true);
		Main.getBBlock(ast.getChunkCoordinates().c(0, 1, 0), w).setType(Material.WARPED_PRESSURE_PLATE, false);
		if (shtrs.size() > 1) {
			bst.setGlowingTag(true);
			bst.setCustomNameVisible(true);
			Main.getBBlock(bst.getChunkCoordinates().c(0, 1, 0), w).setType(Material.WARPED_PRESSURE_PLATE, false);
	...
	private void swpDayNght() {
		...
		if (isDay) {
			...
			for (final Mobber mb : sis) {
				mb.ind.setGlowingTag(true);
				final Block b = Main.getBBlock(mb.ind.getChunkCoordinates(), w);
	...
	private void gameScore(final Shooter sh) {
		...
		for (final Mobber m : sis) {
			t.addEntry(m.ind.getUniqueIDString());
		}
		a.addEntry(ast.getUniqueIDString());
		b.addEntry(bst.getUniqueIDString());
	}
	...
	public void rmvSpnr(final Shooter pl) {
		...
		chngMn(pl, 150);
		if (mb != null) {
			Main.getBBlock(mb.ind.getChunkCoordinates(), w).setType(Material.CRYING_OBSIDIAN, false);
			mb.ind.setGlowingTag(false);
		}
		for (final Entry<Shooter, Mobber> e : dfs.entrySet()) {
			if (e.getValue().ind.getChunkCoordinates().equals(mb.ind.getChunkCoordinates())) {
				e.getKey().inv.getHolder().closeInventory();
			}
		}
		for (final Mobber m : sis) {
			if (m.ind.isCurrentlyGlowing()) {
				return;
			}
		}
	...
	public void wrngWire(final Shooter sh) {
		for (final Entry<Shooter, Mobber> e : dfs.entrySet()) {
			if (e.getKey().equals(sh)) {
				e.getValue().spwnMb(Main.getBBlock(e.getValue().ind.getChunkCoordinates(), w));
			}
		}
	}
	...
	public void endSps() {
		for (final Mobber m : sis) {
			if (m != null) {
				Main.getBBlock(m.ind.getChunkCoordinates(), w).setType(Material.AIR, false);
				m.ind.killEntity();
				m.cancel();
			}
		}
		Main.getBBlock(ast.getChunkCoordinates().c(0, 1, 0), w).setType(Material.AIR, false);
		ast.killEntity();
		Main.getBBlock(bst.getChunkCoordinates().c(0, 1, 0), w).setType(Material.AIR, false);
		bst.killEntity();
	}
	...
	public void hrtSt(final boolean isA, final char l) {
		...
		if (isA) {
			apc -= dmg;
			if (apc > 0) {
				ast.setCustomName(IChatBaseComponent.a("§7Точка §bA§7: §d" + apc + "%"));
				for (final Shooter sh : shtrs.keySet()) {
					final Player p = (Player) sh.inv.getHolder();
				...
				final Block b = Main.getBBlock(ast.getChunkCoordinates().c(0, 1, 0), w);
			...
			bpc -= dmg;
			if (bpc > 0) {
				bst.setCustomName(IChatBaseComponent.a("§7Точка §6B§7: §d" + bpc + "%"));
				for (final Shooter sh : shtrs.keySet()) {
					final Player p = (Player) sh.inv.getHolder();
					p.playSound(ast.getBukkitEntity().getLocation(), Sound.BLOCK_SCULK_SENSOR_CLICKING, 40f, 1f);
				...
				final Block b = Main.getBBlock(bst.getChunkCoordinates().c(0, 1, 0), w);
				b.getWorld().playSound(b.getLocation(), Sound.BLOCK_CONDUIT_DEACTIVATE, 40f, 2f);
